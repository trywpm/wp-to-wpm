...
move_html() {
    ...
    mc mv "$source_full_path" "$dest_full_path"
}
...
while true; do
    echo "=================================================="
    echo "$(date) - Starting new migration cycle for .html files."

    source_find_path="$source_s3"

    first_file=$(mc find "$source_find_path" --name "*.html" --ignore "_t*" | head -n 1)

    if [ -z "$first_file" ]; then
        echo "No .html files found to move. Sleeping for 30 seconds..."
        sleep 30
        continue
    fi
    ...
    mc find "$source_find_path" --name "*.html" --ignore "_t*" | xargs -n 1 -P 20 -I {} bash -c 'move_html "{}"'
done
